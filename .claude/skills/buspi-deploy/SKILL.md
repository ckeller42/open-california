...
- The web UI is also served over **HTTPS on the tailnet** via `tailscale serve` (proxying
  `127.0.0.1:8088`) at `https://<pi-name>.<tailnet>.ts.net/` — tailnet-only, valid LE cert,
  persistent across reboots. Manage on buspi (needs sudo): `sudo tailscale serve status`,
  disable with `sudo tailscale serve --https=443 off`. **Never `tailscale funnel`** it (that
  publishes the unauthenticated, write-capable UI to the public internet). User-facing setup

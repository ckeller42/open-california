...
# 2) a live read/set needs the BLE slot — stop the daemon first
ssh buspi 'sudo systemctl stop calictl && sleep 1'
...
ssh buspi 'sudo systemctl start calictl && sleep 2 && systemctl is-active calictl'
```
...
- `sudo` on buspi needs a password (set your own — do **not** keep the Pi default; never commit it); secrets in `/etc/buspi/*.env` (root, 0600).
